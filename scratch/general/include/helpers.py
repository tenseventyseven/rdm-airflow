...
def _fetch_data(url: str, model: Type[BaseModel]):
    response = httpx.get(url)
    response.raise_for_status()
    return TypeAdapter(model).validate_python(response.json())


def _post_data(url: str, payload: dict, model: Type[BaseModel]):
...
def make_dataset(project_id: str, dataset_id: str) -> Dataset:
    ...
    return _post_data(url, payload, Dataset)
